...
from hypergo.metrics.datalink import DataLinkMetric, Meter
...
def collect_metrics(func: Callable[..., Any]) -> Callable[..., Any]:
    ...
    def wrapper(self: Any, data: Any) -> Any:
        ...
        for custom_metrics in custom_metrics_metadata:
            for metric_callback in DataLinkMetric.get_metrics_callback(package=custom_metrics.package,
        ...
        meter: Meter = DataLinkMetric.get_meter(name=function_name)
        for metric_callback, value in metric_callbacks.items():
            DataLinkMetric.send(meter=meter, metric_name=metric_callback.__name__, metric_result=metric_callback(value),
