...
class CustomMetrics(ExecutionTimeMetrics, ResourceUsageMetrics):
    ...
    def cpu_usage(popped_result: MetricResult = None) -> MetricResult:
        ...
        return MetricResult(unit="percent", name="cpu_usage",
                            value=abs(process.cpu_percent(interval=0.1) - popped_result.value))
    ...
    @staticmethod
    def disk_io_bytes(popped_result: Tuple[MetricResult] = None) -> Tuple[MetricResult]:
        """Record disk I/O usage in bytes by function"""
        if not popped_result:
            disk_io = psutil.disk_io_counters()
            return (MetricResult(unit="bytes", name="read_bytes", value=disk_io.read_bytes),
        ...
        return (MetricResult(unit="bytes", name="read_bytes", value=disk_io.read_bytes-read_bytes.value),
                MetricResult(unit="bytes", name="write_bytes", value=disk_io.write_bytes-write_bytes.value))

    @staticmethod
    def disk_io_time(popped_result: Tuple[MetricResult] = None) -> Tuple[MetricResult]:
        """Record disk I/O usage in seconds by function"""
        if not popped_result:
            disk_io = psutil.disk_io_counters()
            return (MetricResult(unit="ms", name="read_time", value=disk_io.read_time),
        ...
        return (MetricResult(unit="seconds", name="read_time", value=(disk_io.read_time-read_time.value)*0.001),
